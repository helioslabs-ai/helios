			...
			<header className="relative overflow-hidden bg-radial-gold border-b border-[#1a1c24]">
				...
				<div className="relative mx-auto max-w-7xl px-4 sm:px-6 pt-10 pb-8">
								...
								<div className="mt-0.5 flex items-center gap-2">
									<a
										href={getOkLinkAddressUrl(
											agents.find((agent) => agent.name === "curator")
												?.address ??
												"0x726cf0c4fe559db9a32396161694c7b88c60c947/token-transfer"
										)}
										target="_blank"
					...
					<p className="mb-6 max-w-5xl text-sm leading-relaxed text-[#94a3b8]">
						Four sovereign AI agents autonomously operate their own wallets, earn yield,
						execute trades, pay each other, and compound capital in a self-sustaining
						DeFi economy via x402 on X Layer. Capital on autopilot.
					</p>
					...
					<div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-px rounded-lg overflow-hidden border border-[#1a1c24]">
						...
						<StatCell
							...
							sub={
								status.lastCycleAt
									? `last ${formatRelativeTime(status.lastCycleAt)}`
									: undefined
							}
						/>
					</div>
		...
		<div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
								...
								{lastCycle.sentinelVerdict && (
									<span
										className={cn(
											...
											lastCycle.sentinelVerdict === "CLEAR"
												? "text-[#10b981]"
												: "text-[#ef4444]"
										)}
									>
			...
			<div className="flex flex-col gap-4 lg:col-span-6">
				...
				<Card className="overflow-hidden h-[22rem] lg:h-[24rem]">
					<OverviewTransactionFeed
						transactions={transactions}
						onShowAll={onOpenTransactionsTab}
					/>
				</Card>
							...
							>
								...
								<td className="px-3 py-2 whitespace-nowrap w-[7.5rem]">
									{isExplorableTxHash(tx.txHash) ? (
										...
										<span
											className="text-[#475569] font-mono"
											title={tx.txHash}
										>
											—
										</span>
...
function YieldCard({ y }: { y: YieldPosition }) {
	return (
		<div className="rounded-lg border border-[#10b981]/20 bg-[#10b981]/5 px-4 py-3 flex items-center justify-between">
			<div>
				<div className="text-[9px] font-mono uppercase tracking-widest text-[#4a5568]">
					Yield
				</div>
	...
	const data = order
		.map((name, i) => ({
			...
			value: Number.parseFloat(
				agents.find((agent) => agent.name === name)?.totalValueUsd ?? "0"
			),
			color: colors[i],
		}))
		.filter((d) => d.value > 0)
		...
		<div>
			...
			{transactions.length === 0 ? (
				<Card className="px-6 py-12 text-center text-[11px] font-mono text-[#334155]">
					No onchain activity recorded yet. The swarm runs on its own schedule — check
					back after the next cycle.
				</Card>
			) : (
							...
							<tbody>
								{visible.map((tx, idx) => (
									...
									>
										...
										<td className="px-4 py-2.5 text-[#94a3b8] capitalize">
											{tx.agent}
										</td>
										<td className="px-4 py-2.5 text-[#64748b] max-w-xs">
											{tx.context}
										</td>
										<td
											className="px-4 py-2.5 text-[#64748b] max-w-xs"
											title={tx.reasoning}
										>
											{tx.reasoning}
										</td>
						...
						<div className="flex items-center gap-6 font-mono">
							<div className="text-right">
								...
								<div className="text-white font-bold">
									${positions.yieldPosition.amountUsdc}
								</div>
							</div>
							<div className="text-right">
								<div className="text-[9px] text-[#4a5568] uppercase tracking-widest">
			...
			<div>
				...
				<div className="rounded-lg border border-[#1a1c24] overflow-hidden">
					{walletAgents.map((agent, index) => (
						...
						>
							<span className="text-sm font-mono capitalize text-white">
								{agent.name}
							</span>
	...
	return (
		<div className="flex flex-col gap-6 max-w-3xl">
			...
			<div className="text-[10px] font-mono text-[#334155] border border-[#1a1c24] rounded-lg px-4 py-3">
				Curator pays Strategist, Sentinel, and Executor ~0.001 USDG per service call via
				EIP-3009 x402 micropayments on X Layer. Every payment is a real onchain transaction.
			</div>
		</div>
	)
}
									...
									>
										...
										<td className="px-4 py-3">
											...
											<div className="mt-0.5 text-[9px] text-[#334155]">
												{entry.curatorAddress.slice(0, 6)}…
												{entry.curatorAddress.slice(-4)}
											</div>
										</td>
